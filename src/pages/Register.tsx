...
const Register = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_hsl(270_80%_60%/0.06)_0%,_transparent_70%)]" />
      <div className="w-full max-w-md p-8 relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">Start predicting CS2 matches today</p>
        </div>

        <div className="rounded-2xl bg-card border border-border/50 p-6 space-y-4">
          <Button variant="outline" className="w-full border-border/50 hover:bg-surface-hover">
          ...
          {[
            { id: "username", label: "Username", type: "text", placeholder: "cs2pro" },
            ...
            <div key={f.id} className="space-y-2">
              ...
              <Input id={f.id} type={f.type} placeholder={f.placeholder} value={form[f.id as keyof typeof form]} onChange={(e) => update(f.id, e.target.value)} className="bg-surface border-border/50" />
            </div>
          ))}

          <Button className="w-full bg-gradient-primary hover:opacity-90" onClick={() => window.location.href = "/dashboard"}>
            Create Account
          </Button>
        </div>
